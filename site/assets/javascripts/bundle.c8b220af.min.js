...
` +
  ...
  `)
	...
	function Qr(e, t) {
		let r = t & 2,
			...
			n = Object.keys(e.terms)
				.filter((p) => !e.terms[p])
				.reduce(
					(p, c) => [...p, x("del", null, (0, Ln.default)(c)), " "],
					[],
				)
				.slice(0, -1),
			i = xe(),
	...
	function Ya(e) {
		...
		return O(
			...e.map((r) =>
				h(r, "change").pipe(m(() => R(`label[for="${r.id}"]`))),
			),
		).pipe(
			Q(R(`label[for="${t.id}"]`)),
			m((r) => ({ active: r })),
