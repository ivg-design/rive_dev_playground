...
export function initializeAssetManager(riveInstance) {
	logger.debug("[INIT] Initializing Asset Manager", {
...
export function setAssetMap(assets) {
	...
	if (assetMap.size > 0) {
		logger.trace(
			"[ASSET_MAP] Asset details:",
			Array.from(assetMap.entries()).map(([name, asset]) => ({
				name,
...
function buildAssetManagerUI() {
	...
	try {
		...
		const sortedAssets = Array.from(assetMap.entries()).sort(
			([nameA], [nameB]) => {
		...
		logger.debug(
			`[UI] Sorted ${sortedAssets.length} assets alphabetically`,
		);
...
function createAssetItem(asset, assetName, index) {
	...
	const fileExtension =
		asset.fileExtension || getExtensionFromName(assetName);
	...
	if (assetHeader) {
		assetHeader.addEventListener("click", (event) => {
			...
			logger.debug(
				`[UI] Asset item ${isExpanded ? "collapsing" : "expanding"}:`,
				assetName,
			);
...
function handleFileReplacement(event, asset, assetName, index) {
	...
	logger.info(
		`[FILE_REPLACE] Replacing asset "${assetName}" with local file: ${file.name}`,
	);
...
function handleUrlReplacement(url, asset, assetName, index) {
	logger.info(
		`[URL_REPLACE] Replacing asset "${assetName}" with URL: ${url}`,
	);
...
function substituteImage(name, url) {
	...
	if (!asset || !url) {
		logger.error(
			`[SUBSTITUTE] Cannot substitute image: asset "${name}" not found or no URL provided`,
		);
		return;
	}

	logger.debug(
		`[SUBSTITUTE] Starting image substitution for "${name}" from URL: ${url}`,
	);
	...
	fetch(url)
		...
		.then((buf) => {
			logger.debug(
				`[SUBSTITUTE] Received ${buf.byteLength} bytes for "${name}"`,
			);
		...
		.catch((error) => {
			logger.error(
				`[SUBSTITUTE] Image decode/replacement error for "${name}":`,
				error,
			);
...
export function clearAssetManager() {
	...
	logger.info(
		`[CLEAR] Asset manager cleared (removed ${previousAssetCount} assets)`,
	);
}
