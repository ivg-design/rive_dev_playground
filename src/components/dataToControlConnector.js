...
function debugTestVMBinding(vmInstance, propertyName = null) {
	...
	if (propertyName) {
		...
		testProperty = vmInstance.properties.find(
			(p) =>
				p.name === propertyName &&
				(p.type === "boolean" ||
					p.type === "number" ||
	...
	logger.trace(
		`Selected test property: ${testProperty.name} (${testProperty.type})`,
	);
	...
	try {
		...
		if (changeWorked) {
			logger.trace(
				"✅ VM binding TEST PASSED - Property accepts changes",
			);
		} else {
			logger.error(
				"❌ VM binding TEST FAILED - Property does not accept changes",
			);
		}
...
export function processDataForControls(parsedData, riveInstance) {
	...
	const activeArtboardName =
		defaultElements.artboardName ||
		(riveInstance.artboard ? riveInstance.artboard.name : null);
	const activeStateMachineNames =
		defaultElements.stateMachineNames || riveInstance.stateMachines || [];
			...
			try {
				...
				const liveInputs =
					typeof riveInstance.stateMachineInputs === "function"
						? riveInstance.stateMachineInputs(smName)
						: [];

				if (liveInputs && liveInputs.length > 0) {
					// Find matching SM in parsed data for reference
					const parsedSM = findStateMachineInParsedData(
						parsedData,
						activeArtboardName,
						smName,
					);
	...
	if (riveInstance.viewModelInstance) {
		logger.info(
			"Found primary ViewModel via riveInstance.viewModelInstance (like exampleIndex.mjs)",
		);
		...
		logger.info(
			`Found ViewModel via riveInstance["${activeArtboardName}"]`,
		);
		mainViewModelInstance = riveInstance[activeArtboardName];
	} else if (
		riveInstance.artboard &&
		riveInstance.artboard.defaultViewModel
	) {
		...
		try {
			...
			if (vmDef && typeof vmDef.defaultInstance === "function") {
				...
				logger.info(
					"Found ViewModel via artboard.defaultViewModel().defaultInstance()",
				);
			}
		} catch (e) {
			logger.warn(
				"Error getting ViewModel via artboard.defaultViewModel():",
				e,
			);
		}
		...
		try {
			...
			if (vmDef) {
				if (typeof vmDef.defaultInstance === "function") {
					mainViewModelInstance = vmDef.defaultInstance();
					logger.info(
						"Got VM from defaultViewModel().defaultInstance()",
					);
				} else if (
					vmDef.instanceCount > 0 &&
					typeof vmDef.getInstance === "function"
				) {
					mainViewModelInstance = vmDef.getInstance(0);
					logger.info(
						"Got VM from defaultViewModel().getInstance(0)",
					);
				}
			}
		} catch (e) {
	...
	if (mainViewModelInstance) {
		logger.info(
			"Successfully found ViewModel instance:",
		...
		if (!bindingResult) {
			logger.warn(
				"🚨 ViewModel binding test FAILED - Controls may not update the animation!",
			);
		} else {
			logger.info(
				"✅ ViewModel binding test PASSED - Controls should update the animation",
			);
			...
			try {
				...
				if (
					mainViewModelInstance.properties.some(
						(p) => p.name === "Pills Active",
					)
				) {
				...
				logger.info(
					"ViewModel instance exposed as window.vm for console debugging",
				);
			} catch (e) {
		...
		const mainVmInfo = {
			...
			blueprintName:
				activeViewModelName || mainViewModelInstance.name || "Unknown",
		...
		if (
			mainViewModelInstance.properties &&
			Array.isArray(mainViewModelInstance.properties)
		) {
			logger.debug(
				`Processing ${mainViewModelInstance.properties.length} properties on main VM`,
			);
			...
			mainViewModelInstance.properties.forEach((prop) => {
				...
				try {
					...
					switch (prop.type) {
						case "boolean":
							vmProperty = mainViewModelInstance.boolean(
								prop.name,
							);
							break;
						case "number":
							vmProperty = mainViewModelInstance.number(
								prop.name,
							);
							break;
						case "string":
							vmProperty = mainViewModelInstance.string(
								prop.name,
							);
							break;
						case "color":
							...
							break;
						case "enumType":
							vmProperty = mainViewModelInstance.enum
								? mainViewModelInstance.enum(prop.name)
								: mainViewModelInstance.string(prop.name);
							break;
						case "trigger":
					...
					if (vmProperty) {
						...
						if (prop.type === "enumType") {
							const parsedVmBlueprint = findViewModelInParsedData(
								parsedData,
								activeArtboardName,
								mainViewModelInstance.name,
							);
							if (parsedVmBlueprint && parsedVmBlueprint.inputs) {
								const parsedInputInfo =
									parsedVmBlueprint.inputs.find(
										(inp) => inp.name === prop.name,
									);
								if (
									parsedInputInfo &&
									parsedInputInfo.enumTypeName
								) {
									propertyEntry.enumTypeName =
										parsedInputInfo.enumTypeName;
									logger.trace(
										`[dataConnector] Main VM prop '${prop.name}': Using enumTypeName '${parsedInputInfo.enumTypeName}' from parser data.`,
									);
									logger.debug(
										`[dataConnector] Main VM prop '${prop.name}': Set enumTypeName to '${parsedInputInfo.enumTypeName}'`,
									);
								} else {
									logger.warn(
										`[dataConnector] Main VM prop '${prop.name}': Did not find enumTypeName in parsed data for blueprint '${mainViewModelInstance.name}'. UI will use property name as fallback for enum list.`,
									);
									logger.debug(
										`[dataConnector] Main VM prop '${prop.name}': No enumTypeName found, using property name '${prop.name}' as fallback`,
									);
									logger.debug(
										`[dataConnector] parsedInputInfo:`,
										parsedInputInfo,
									);
								...
								logger.warn(
									`[dataConnector] Main VM prop '${prop.name}': Could not find parsed VM blueprint '${mainViewModelInstance.name}' or its inputs to get enumTypeName. UI will use property name as fallback.`,
								);
								propertyEntry.enumTypeName = prop.name;
							}
						}
			...
			const nestedVmProps = mainViewModelInstance.properties.filter(
				(p) => p.type === "viewModel",
			);
			...
			nestedVmProps.forEach((vmProp) => {
				try {
					...
					const nestedVmInstance = mainViewModelInstance.viewModel(
						vmProp.name,
					);
					...
					if (nestedVmInstance) {
						...
						const nestedVmInfo = buildNestedVMControls(
							nestedVmInstance,
							vmProp.name,
							parsedData,
						);
		...
		if (parsedData && parsedData.artboards) {
			const artboard = parsedData.artboards.find(
				(a) => a.name === activeArtboardName,
			);
			if (
				artboard &&
				artboard.viewModels &&
				artboard.viewModels.length > 0
			) {
				...
				if (mainVM) {
					...
					if (
						mainVM.nestedViewModels &&
						mainVM.nestedViewModels.length > 0
					) {
						mainVM.nestedViewModels.forEach((nestedVM) => {
							placeholderInfo.nestedViewModels.push({
								...
								properties: (nestedVM.inputs || []).map(
									(input) => ({
										name: input.name,
										type: input.type,
										value: input.value,
	...
	logger.info(
		`Final viewModelControls structure has ${viewModelControls.length} entries`,
	);
		...
		setTimeout(() => {
			...
			if (allEnums && allEnums.length > 0) {
				allEnums.forEach((enumDef, index) => {
					logger.info(
						`🔍 Enum ${index + 1}: Name='${enumDef.name}', Values:`,
						enumDef.values,
					);
				});
			}
...
function buildNestedVMControls(vmInstance, instanceName, parsedData) {
	...
	if (vmInstance.properties && Array.isArray(vmInstance.properties)) {
		logger.debug(
			`[buildNestedVMControls] VM '${instanceName}' has ${vmInstance.properties.length} properties`,
		);
		logger.debug(
			`[buildNestedVMControls] VM '${instanceName}' available methods:`,
			{
		...
		vmInstance.properties.forEach((prop) => {
			...
			try {
				...
				logger.debug(
					`[buildNestedVMControls] Processing property '${prop.name}' of type '${prop.type}' in VM '${instanceName}'`,
				);
				...
				switch (prop.type) {
					...
					case "enumType":
						logger.debug(
							`[buildNestedVMControls] Attempting to access enum property '${prop.name}' via vmInstance.enum()`,
						);
						if (typeof vmInstance.enum === "function") {
							try {
								liveProperty = vmInstance.enum(prop.name);
								logger.debug(
									`[buildNestedVMControls] Successfully got enum property '${prop.name}':`,
									liveProperty,
								);
							} catch (e) {
								logger.warn(
									`[buildNestedVMControls] Error accessing enum property '${prop.name}' via vmInstance.enum():`,
									e,
								);
								// Fallback to string method
								if (typeof vmInstance.string === "function") {
									try {
										liveProperty = vmInstance.string(
											prop.name,
										);
										logger.debug(
											`[buildNestedVMControls] Fallback: got enum property '${prop.name}' via string():`,
											liveProperty,
										);
									} catch (e2) {
										logger.error(
											`[buildNestedVMControls] Fallback also failed for '${prop.name}':`,
											e2,
										);
									}
								}
							...
							logger.warn(
								`[buildNestedVMControls] vmInstance.enum is not a function for property '${prop.name}', trying string fallback`,
							);
							liveProperty = vmInstance.string
								? vmInstance.string(prop.name)
								: null;
						}
						break;
					case "trigger":
				...
				if (liveProperty) {
					logger.debug(
						`[buildNestedVMControls] Successfully created liveProperty for '${prop.name}' in VM '${instanceName}'`,
					);
					...
					if (prop.type === "enumType") {
						...
						if (
							parsedData &&
							parsedData.allViewModelDefinitionsAndInstances &&
							vmInstance.name
						) {
							const foundVmDef =
								parsedData.allViewModelDefinitionsAndInstances.find(
									(def) =>
										def.blueprintName === vmInstance.name,
								);
							// The `inputs` we need were stored under `blueprintProperties` by the parser for the definition
							if (foundVmDef)
								parsedVmBlueprint = {
									name: foundVmDef.blueprintName,
									inputs: foundVmDef.blueprintProperties,
								};
						}
						...
						if (parsedVmBlueprint && parsedVmBlueprint.inputs) {
							...
							if (
								...
								nestedPropEntry.enumTypeName =
									parsedInputInfo.enumTypeName;
								logger.trace(
									`[dataConnector] Nested VM prop '${instanceName}.${prop.name}': Using enumTypeName '${parsedInputInfo.enumTypeName}' from parser data (blueprint: ${vmInstance.name}).`,
								);
							} else {
								logger.warn(
									`[dataConnector] Nested VM prop '${instanceName}.${prop.name}': Did not find enumTypeName in parsed data for blueprint '${vmInstance.name}'. UI will use property name as fallback.`,
								);
								nestedPropEntry.enumTypeName = prop.name;
							}
						} else {
							logger.warn(
								`[dataConnector] Nested VM prop '${instanceName}.${prop.name}': Could not find parsed blueprint definition '${vmInstance.name}' or its properties. UI will use property name as fallback.`,
							);
							nestedPropEntry.enumTypeName = prop.name;
						}
					}
					...
					logger.debug(
						`[buildNestedVMControls] Added nested property ${instanceName}.${prop.name} (${prop.type}) to result`,
					);
				} else {
					logger.warn(
						`[buildNestedVMControls] Failed to get liveProperty for '${prop.name}' in VM '${instanceName}' - property will be skipped`,
					);
				}
			} catch (e) {
				logger.warn(
					`[buildNestedVMControls] Error accessing nested property ${prop.name}:`,
					e,
				);
			}
		});
		...
		const nestedVmProps = vmInstance.properties.filter(
			(p) => p.type === "viewModel",
		);
		if (nestedVmProps.length > 0) {
			logger.trace(
				`Found ${nestedVmProps.length} deeper nested VMs in ${instanceName}`,
			);
			...
			nestedVmProps.forEach((vmProp) => {
				try {
					...
					if (deeperVmInstance) {
						const deeperVmInfo = buildNestedVMControls(
							deeperVmInstance,
							vmProp.name,
							parsedData,
						);
					...
					logger.warn(
						`Error processing deeper nested VM ${vmProp.name}:`,
						e,
					);
				}
			});
 ...
 */
function findStateMachineInParsedData(
	parsedData,
	artboardName,
	stateMachineName,
) {
...
function findViewModelInParsedData(parsedData, artboardName, viewModelName) {
	...
	if (viewModelName) {
		return artboard.viewModels.find(
			(vm) => vm.sourceBlueprintName === viewModelName,
		);
	}
